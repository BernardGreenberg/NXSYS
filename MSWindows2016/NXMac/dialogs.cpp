...
static char DemoFilter[] = "Interlocking Demos (*.xdo)\0*.xdo\0All Files (*.*)\0*.*\0\0";
static char ScriptFilter[] = "NXScript scripts (*.nxs)\0*.nxs\0All Files (*.*)\0*.*\0\0";
static char szFilter[] = "Track Layouts (*.tko)\0*.TKO\0Expr Code (*.TRK)\0*.TRK\0Both (.TRK/.TKO)\0*.TRK;*.TKO\0All Files (*.*)\0*.*\0\0";
static char *InterpFilter = "Interpreted Xlkgs (*.trk)\0*.TRK\0All Files (*.*)\0*.*\0\0";
...
DLGPROC_DCL about_box_DlgProc(HWND dialog, unsigned message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		sprintf_s<COUNTOF(sversion)>
			(sversion, "Version %d.%d.%d.%d (MS Windows 32 bit)", fv1, fv2, fv3, fv4);
		...
		UINT cbLang;
		WORD* langInfo;
		VerQueryValue(vdp, "\\VarFileInfo\\Translation",
