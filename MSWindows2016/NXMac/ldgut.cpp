...
const int DrawingCountMax = 100;
Drawing* Drawings[DrawingCountMax];
...
void DrawingSetPageSize (int width, int height, int canonm, int flags) {
    ...
    NCellH = new_ncellh;
    DNCellH = 2*NCellH;
    for (int dno = 0; dno < DrawingCount; dno++)
	Drawings[dno]->PlaceOnPage();
}
...
public:
    ...
    BOOL Lastp ();
    int Rodentate (WPARAM wParam, WORD x, WORD y, Drawing &);
};
...
GNode * CreateGNode (LNode * ln, GNode * parent, Drawing& d) {
	...
	n->Statep = &(ln->State);
	n->type = CT_CONST;
	strcpy (n->text, *n->Statep ? "TRUE" : "FALSE");
	n->Thread(parent);
	n->width = n->height = 1;
	return n;
    }
	...
	n->type = CT_FRONT;
	n->Statep = &(ln->State);
	strcpy (n->text, RlysymPRep (((Relay *) ln)->RelaySym));
	n->Thread(parent);
	n->width = n->height = 1;
...
raf:	FatalAppExit (0, "LDRAW root alloc fails.");
    root.type = CT_AND;
    GNode& relay = *new GNode;
	if (&relay == NULL)
	    goto raf;
    Relay * r = (Relay *) ln;
    strcpy (relay.text, RlysymPRep (r->RelaySym));
    ...
    D->yorg = DNCellH/2;
    D->relay = r;
    GNode * gn = CreateGNode (exp, &root, *D);
...
void GNode::DrawCoil (HDC dc, Scord scx, Scord scy, int timer_p) {
    ...
    txr.left = txr.right =  txr.top = txr.bottom = 0;
    DrawText (dc, text, strlen(text), &txr,
	      DT_TOP | DT_LEFT |DT_SINGLELINE| DT_NOCLIP|DT_CALCRECT);
    txr.left = scx;
    txr.right = scx + CellW;
    if (statereport) {
...
void GNode::DrawContactLabel (HDC dc, Scord scx, Scord scy) {
    ...
    DrawText (dc, text, strlen(text), &txr, flgs);
}
...
got_it:
    ...
    d->ycord = y - (DD->miny-1);
    d->PlaceOnPage();
    if (DrawingCount >= DrawingCountMax)
	FatalAppExit (0, "Relay Graphics: Drawing count overflow");
    Drawings[DrawingCount++] = d;
    DD = NULL;
    LastDrawingX = d->x;
    LastDrawingY = d->y;
...
int RelayGraphicsMouse(WPARAM wParam, WORD x, WORD y) {
    if (DrawingCount == 0) {
	if (G != NULL)
	    return G->Rodentate (wParam, x, y, *DD);
	return 0;
    }
    for (int i = 0; i < DrawingCount; i++)
	if (Drawings[i]->root->Rodentate (wParam, x, y, *Drawings[i]))
	    return 1;
    return 0;
}

int GNode::Rodentate (WPARAM wParam, WORD x, WORD y, Drawing& d) {
    if (Child == NULL) {
	if (stick)
	    return 0;
	Scord scx =  CellW*gridx+ d.x + xoff;
	Scord scy = CellH*gridy+ d.y;
	if (y > scy - CellH/2 && y < scy + CellH/2 &&
	    x >= scx && x <= scx + CellW)
	    if (DrawRelayFromName (text))
		return 1;
	return 0;
	}
    for (GNode*sn = Child; sn != NULL; sn = sn->Next)
	if (sn->Rodentate (wParam, x, y, d))
	    return 1;
    return 0;
}
