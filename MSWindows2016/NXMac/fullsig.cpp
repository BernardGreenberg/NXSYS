...
SigHead::SigHead(const char* lights, const  char* plate) {
	strcpy(Lights, lights);
	strcpy(Plate, plate);
	State = 'X';
	height = (short)strlen(lights);
}
...
void Signal::WinDisp(HDC dc, int lights_only) {
	...
	for (int i = 0; i < HeadCount; i++) {
		y = Heads[i]->DisplayWin(this, xc, y, dc, lights_only, stp);
...
int SigHead::DisplayWin(Signal* S,
	...
	if (Plate[0]) {
		if (!lights_only)
			DrawPlate(S, dc, xc, y, 2 * hw, Plate);
		y += 2 * hw;
	}
