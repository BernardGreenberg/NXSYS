...
SetHelpFilePath () {
    ...
    fnsplit (HelpPath, drive, dir, NULL, NULL);
...
#ifdef NXSYSMac  // why isn't this crapola in AppDelegate?
...
static int umsgcmn (va_list ap, const char * ctlstr, UINT ctl) {
    char buf [500];
    vsprintf (buf, ctlstr, ap);
    return MessageBox (G_mainwindow, buf, app_name, ctl);
}
...
#define NxsysAppAbortMsg \
 ...
 "Logic simulation of this scenario can proceed no further.\r\n" \
...
void NxsysAppAbort (int reserved, const char* message) {
    char buf[1024];
    sprintf(buf, NxsysAppAbortMsg, message);
    longjmp (S_JmpBuf, MessageBox (G_mainwindow, buf, PRODUCT_NAME " Layout fatal error", MB_YESNOCANCEL | MB_ICONSTOP));
};
#ifndef NXSYSMac
