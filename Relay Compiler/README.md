...
Second, it is possible to ***run X86-compiled relay code in the Arm application version with its native simulation***. If you try to load such an interlocking into NXSYS (i.e., the Arm build) it will do so without need for being told. The simulation is extremely efficient and robust, as both the simulator and the two disassemblers are acutely aware of the limited output vocabulary of the compiler, and exploit it in time and space.  The title bar will say **X86/Sim** when this is in effect.
